...
import (
	"order/app/Http/Controllers/Example"
	"order/app/Http/Controllers/Example/WebSocket"
	UserList "order/app/Http/Controllers/User/List"
	...
	"order/app/Http/Controllers/Order/Detail"
	"order/app/Http/Controllers/Order/List"
	"order/app/Http/Controllers/User/Base"
...
func ExampleApi(route *gin.Engine) (apis *gin.RouterGroup) {
	// api分组路由
	//	// 验证登陆
	//	HttpServer.Use(Middlewares.CheckLogin)
	api := route.Group("/api/", Middlewares.HttpCorsApi)
	{
		user := api.Group("/user/")
		{
			...
			list := user.Group("/list/", Middlewares.HttpLimiter(2), Middlewares.CheckLogin, Example.VerifyExample)
			{
				list.POST("info", UserList.Info)
				base.POST("active", UserList.Info)
		...
		{
			test := order.Group("/test/", Middlewares.HttpLimiter(2), Example.VerifyExample)
			{
				test.GET("detail", Detail.Test)
				test.GET("list", List.Test)
			}
		...
		{
			test := example.Group("/test/", Middlewares.HttpLimiter(2), Example.VerifyExample, Example.VerifyExample)
			{
				test.GET("test1", Example.Test)
				test.POST("login", Example.Login)
				test.POST("refresh_token", Example.RefreshToken)
			}
